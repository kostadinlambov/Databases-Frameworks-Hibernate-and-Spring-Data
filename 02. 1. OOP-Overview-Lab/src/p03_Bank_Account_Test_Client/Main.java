...
public class Main {
    private static HashMap<Integer, BankAccount> bankAccountMap = new HashMap<>();
    ...
    private static void tryToPrint(Integer id) {
        if(bankAccountMap.containsKey(id)){
            System.out.println(bankAccountMap.get(id));
        }else{
    ...
    private static void withdrawSum(Integer id, Double amount) {
        if(bankAccountMap.containsKey(id)){
            bankAccountMap.get(id).withdraw(amount);
        }else{
            System.out.println("Account does not exist");
        }
    }
